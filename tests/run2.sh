...
if [ $# -eq 0 ]; then
    ...
    BYTEPS_SOCKET_PATH="/tmp/socket_1" taskset -c 32-63  bash run_byteps_test_p2p.sh joint 0   &
else
    ...
    BYTEPS_SOCKET_PATH="/tmp/socket_5" taskset -c 32-63  bash run_byteps_test_p2p.sh joint 1   &
fi
wait
