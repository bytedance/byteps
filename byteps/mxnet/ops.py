...
def allreduce(tensor, average=True, name=None):
    ...
    all the BytePS processes. The input tensor is not modified.
    ...
    if isinstance(name, string_types):
        check_call(MPI_MXNET_LIB_CTYPES.byteps_mxnet_allreduce_async(c_in,
...
def allgather(tensor, name=None):
    ...
    all other BytePS processes. The input tensor is not modified.
    ...
    Returns:
        ...
        first dimensions of the tensors in different BytePS processes.
    """
    ...
    c_out = output.handle
    if isinstance(name, string_types):
        check_call(MPI_MXNET_LIB_CTYPES.byteps_mxnet_allgather_async(c_in,
                   c_out, c_str(name)))
    else:
        check_call(MPI_MXNET_LIB_CTYPES.byteps_mxnet_allgather_async(c_in,
                   c_out, name))
...
def broadcast(tensor, root_rank, name=None):
    ...
    tensor on all other BytePS processes. The input tensor is not modified.

    The broadcast operation is keyed by the name. If name is not provided, an
    incremented auto-generated name is used. The tensor type and shape must be
    the same on all BytePS processes for a given name. The broadcast will not
    start until all processes are ready to send and receive the tensor.
    ...
    c_out = output.handle
    if isinstance(name, string_types):
        check_call(MPI_MXNET_LIB_CTYPES.byteps_mxnet_broadcast_async(c_in,
...
def broadcast_(tensor, root_rank, name=None):
    """
    A function that broadcasts the input tensor on root rank to the same input
    tensor on all other BytePS processes. The operation is performed in-place.
