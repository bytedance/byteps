...
std::unique_ptr<Compressor> CompressorRegistry::Create(const kwargs_t& kwargs,
...
#endif
  auto tensor_size = HyperParamFinder<unsigned>(kwargs, "tensor_size", true,
  ...
  for (auto& type : types) {
    ...
    if (iter != kwargs.end()) {
      ...
      BPS_LOG(DEBUG) << "Compressor create compressor: " << iter->second 
      << ", compressed size: " << size
      << ", tensor size: " << tensor_size << std::endl;
      return ctor(kwargs, tensor_size * sizeof(float), dtype);
    }
  }
