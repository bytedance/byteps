...
class Compressor {
 public:
  Compressor(size_t size, DataType dtype)
      : _size(size), _dtype(dtype), _buf(new byte_t[size]), _convert_buf(new byte_t[size]) {
  ...
  /*!
  * \brief convert FP16 gradients to FP32 
  * \param dst the FP32 gradients after convertion
  * \param src the FP16 gradients before convertion
  * \param len the number of bytes in src
  */
  void FP16toFP32(void* dst, void* src, size_t len);
  /*!
  * \brief convert FP32 gradients to FP16
  * \param dst the FP16 gradients after convertion
  * \param src the FP32 gradients before convertion
  * \param len the number of bytes in src
  */
  void FP32toFP16(void* dst, void* src, size_t len);

  tensor_t FP16TensortoFP32(tensor_t grad);
  tensor_t FP32TensortoFP16(tensor_t grad);
 ...
 protected:
  ...
  /*! \brief if FP16 is used at the workers */
  bool _worker_fp16 = false;
  DataType _dtype;
