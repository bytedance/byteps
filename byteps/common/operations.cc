...
void byteps_lazy_init() {
  ...
  if (BytePSGlobal::IsCrossPcieSwitch() || BytePSGlobal::IsDistributed()) {
    ...
    //func.push_back(CompressCopyDevice2HostLoop);
    if (BytePSGlobal::IsRootDevice()) {
      // PUSH can be a real push in distributed mode
      // Or a dummy barrier in cross-pcie-switch mode
      size_t push_threads = 1;
      if (getenv("BYTEPS_PUSH_THREADS")) {
        push_threads = atoi(getenv("BYTEPS_PUSH_THREADS"));
      }
  ...
  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    ...
    //func.push_back(CoordinateNoneLoop);
    func.push_back(CoordinateReduceLoop);
    func.push_back(CoordinateIntraGatherLoop);
    func.push_back(CoordinateIntraBroadcastLoop);
    func.push_back(CoordinateIntraReducescatterLoop);
    func.push_back(CoordinateIntraAllgatherLoop);
    func.push_back(CoordinateIntraAlltoallLoop);
    func.push_back(CoordinateIntraReduceLoop);
...
Status EnqueueIntraTensor(BPSContext &context, std::shared_ptr<Tensor> input,
                     std::shared_ptr<Tensor> output,
                     std::shared_ptr<ReadyEvent> ready_event, const int device,
                     const int priority, const int version,
                     StatusCallback callback,
  ...
  e->total_partnum = 1;
  e->len = e->tensor->size();
  e->intra_comm_root_rank = context.intra_comm_root_rank;
  ...
  // we don't partition tensors for intra-node reduce
  auto task = e;
  task->key = context.key_list[0];
  BPS_CHECK(task->tensor_name != "");
...
void InitTensor(BPSContext &context, size_t size, int dtype, void *cpubuff) {
  ...
  while (accumulated < size) {
    ...
    BPS_LOG(INFO) << "accumulated=" << accumulated << ", size=" << size;
...
void InitCPUCompressTensor(BPSContext &context, size_t size, int dtype, void *cpubuff) {
  std::lock_guard<std::mutex> lock(context.init_mutex);
  if (context.initialized) {
    return;
  }
  ...
  BPS_CHECK_GT(size, 0) << "init tensor size not larger than 0";
  // Get metadata
  // no tensor partition for CPU compression
  auto bound = 1073741824;
  auto &name = context.tensor_name;
  context.buff_len = size;
  size_t accumulated = 0;
  ...
  while (accumulated < size) {
    ...
    if (BytePSGlobal::IsDistributed() && BytePSGlobal::IsRootDevice()) {
      ...
      ps::SArray<char> vals(data + accumulated, len, false);
      // cmd type
      // Server for compression will always receive the data with the type of BYTEPS_FLOAT32
  ...
  if (!context.kwargs.empty() && BytePSGlobal::IsDistributed() &&
    ...
    for (auto key : key_list) {
      ...
      int cmd = GetCommandType(RequestType::kCompressedPushPull, BYTEPS_FLOAT32);
  ...
  BPS_LOG(TRACE) << "Finish CPU Compression Init " << name << ", size=" << size
                 << ", parts=" << key_list.size();
}


void InitIntraTensor(BPSContext &context, size_t size, int dtype, void *cpubuff, int intra_root) {
  std::lock_guard<std::mutex> lock(context.init_mutex);
  if (context.initialized) {
    return;
  }
  ...
  BytePSGlobal::SetProfileFlag(&context);
  context.local_rank = BytePSGlobal::GetLocalRank();
  context.intra_comm_root_rank = intra_root;
...
bool IsTensorDeclared(const std::string &name, const size_t size) {
  return BytePSGlobal::IsTensorDeclared(name, size);
}
...
std::shared_ptr<std::vector<QueueType>> GetCPUCompressQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();
  ...
  if (BytePSGlobal::IsDistributed()) {
    if (BytePSGlobal::IsRootDevice()) {
      queue_list->push_back(PULL);
      queue_list->push_back(DECOMPRESS);
    }
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraGatherQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_GATHER);
  } else {
    queue_list->push_back(COORDINATE_INTRA_GATHER);
    queue_list->push_back(INTRA_GATHER);
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraBroadcastQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_BROADCAST);
  } else {
    queue_list->push_back(COORDINATE_INTRA_BROADCAST);
    queue_list->push_back(INTRA_BROADCAST);
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraReducescatterQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_REDUCESCATTER);
  } else {
    queue_list->push_back(COORDINATE_INTRA_REDUCESCATTER);
    queue_list->push_back(INTRA_REDUCESCATTER);
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraAllgatherQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_ALLGATHER);
  } else {
    queue_list->push_back(COORDINATE_INTRA_ALLGATHER);
    queue_list->push_back(INTRA_ALLGATHER);
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraAlltoallQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_ALLTOALL);
  } else {
    queue_list->push_back(COORDINATE_INTRA_ALLTOALL);
    queue_list->push_back(INTRA_ALLTOALL);
  }
...
std::shared_ptr<std::vector<QueueType>> GetIntraReduceQueueList(int device) {
  auto queue_list = std::make_shared<std::vector<QueueType>>();

  if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
    queue_list->push_back(INTRA_REDUCE);
  } else {
    queue_list->push_back(COORDINATE_INTRA_REDUCE);
    queue_list->push_back(INTRA_REDUCE);
  }
