...
BytePSScheduledQueue::BytePSScheduledQueue(QueueType type) {
  ...
  switch (_qt) {
    ...
    case INTRA_REDUCE:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraReduceTable();
      }
      break;
    case INTRA_GATHER:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraGatherTable();
      }
      break;
    case INTRA_BROADCAST:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraBroadcastTable();
      }
      break;
    case INTRA_REDUCESCATTER:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraReducescatterTable();
      }
      break;
    case INTRA_ALLGATHER:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraAllgatherTable();
      }
      break;
    case INTRA_ALLTOALL:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
        _rt = BytePSGlobal::GetIntraAlltoallTable();
      }
      break;
    case PCIE_REDUCE:
    ...
    case COPYH2D:
      ...
      break;
    case COMPRESS_COPYH2D:
      if (!BytePSGlobal::IsRootDevice()) {
        _rt = BytePSGlobal::GetCompressCopyTable();
      }
      break;
    case BROADCAST:
      if (BytePSGlobal::GetNccl()->IsSignalRoot()) {
