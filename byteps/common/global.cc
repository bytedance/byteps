...
std::string BytePSGlobal::_intra_compressor = "";
std::string BytePSGlobal::_inter_compressor = "";
std::string BytePSGlobal::_compressor_error_feedback = "";
float BytePSGlobal::_compressor_k = 0.01;
...
ReadyTable* BytePSGlobal::_none_table;
ReadyTable* BytePSGlobal::_reduce_table;
ReadyTable* BytePSGlobal::_intra_reduce_table;
ReadyTable* BytePSGlobal::_intra_gather_table;
ReadyTable* BytePSGlobal::_intra_broadcast_table;
ReadyTable* BytePSGlobal::_intra_reducescatter_table;
ReadyTable* BytePSGlobal::_intra_allgather_table;
ReadyTable* BytePSGlobal::_intra_alltoall_table;
...
ReadyTable* BytePSGlobal::_compress_copy_table;
...
void BytePSGlobal::Init() {
  ...
  if (getenv("BYTEPS_INTRA_COMPRESSOR")) {
    _intra_compressor = getenv("BYTEPS_INTRA_COMPRESSOR");
  }
  if (getenv("BYTEPS_INTER_COMPRESSOR")) {
    _inter_compressor = getenv("BYTEPS_INTER_COMPRESSOR");
  }
  if (getenv("BYTEPS_COMPRESSOR_ERROR_FEEDBACK")) {
    _compressor_error_feedback = getenv("BYTEPS_COMPRESSOR_ERROR_FEEDBACK");
  }
  if (getenv("BYTEPS_COMPRESSOR_K")) {
    _compressor_k = atof(getenv("BYTEPS_COMPRESSOR_K"));
  }
  ...
  if (_is_root_device) {
    ...
    _compress_copy_table = new ReadyTable(1, "COMPRESS_COPY");
  }
  ...
  if (_nccl_manager->IsSignalRoot()) {
    _none_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_NONE");
    _reduce_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_REDUCE");
    _intra_reduce_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_REDUCE");
    _intra_gather_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_GATHER");
    _intra_broadcast_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_BROADCAST");
    _intra_reducescatter_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_REDUCESCATTER");
    _intra_allgather_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_ALLGATHER");
    _intra_alltoall_table = new ReadyTable(GetPcieSwitchSize() - 1, "NCCL_INTRA_ALLTOALL");
    _broadcast_table =
...
void BytePSGlobal::Shutdown() {
  ...
  if (_intra_reduce_table) {
    delete _intra_reduce_table;
    _intra_reduce_table = NULL;
  }
  if (_intra_gather_table) {
    delete _intra_gather_table;
    _intra_gather_table = NULL;
  }
  if (_intra_broadcast_table) {
    delete _intra_broadcast_table;
    _intra_broadcast_table = NULL;
  }
  if (_intra_reducescatter_table) {
    delete _intra_reducescatter_table;
    _intra_reducescatter_table = NULL;
  }
  if (_intra_allgather_table) {
    delete _intra_allgather_table;
    _intra_allgather_table = NULL;
  }
  if (_intra_alltoall_table) {
    delete _intra_alltoall_table;
    _intra_alltoall_table = NULL;
  }
  ...
  if (_compress_copy_table) {
    delete _compress_copy_table;
    _compress_copy_table = NULL;
  }
...
bool isIntraComm(const std::string &name) {
  std::string comm_type = name.substr(0, name.find("."));
  return comm_type.find("_intra_") != std::string::npos;
}

bool BytePSGlobal::IsTensorDeclared(const std::string& name, const size_t size) {
  std::lock_guard<std::mutex> lock(_context_mutex);
  if (_name_to_cxt.find(name) == _name_to_cxt.end()) {
    ...
    // compressor
    if (_inter_compressor != "" && name.find("cpu_compress") != std::string::npos) {
      std::unordered_map<std::string, std::string> kwargs;
      kwargs["compressor_type"] = _inter_compressor;
      if (_compressor_error_feedback != "") {
        kwargs["ef_type"] = _compressor_error_feedback;
      }
      kwargs["compressor_k"] = std::to_string(_compressor_k);
      kwargs["tensor_size"] = std::to_string(size);
...
void BytePSGlobal::RegisterCompressor(
  ...
  BPS_CHECK(_name_to_cxt.find(name) != _name_to_cxt.end())
      << name << " is not initialized";
  _name_to_cxt[name].kwargs = std::move(kwargs);
  std::cout << "RegisterCompressor compressor: " << std::endl;
}
...
void BytePSGlobal::EmitTensorReadyTrace(std::ostream* os, const BPSCommTime* ret,
                             BytePSContext* ctxt) {
  if (ret->tensor_ready_t <= 0)
    return;
  
  std::string tid = "tensor_ready";
  std::string para_name = "TensorReady";
  std::string para_name_type = para_name + "." + ctxt->tensor_name;
  (*os) << "        {\n"
        << "            \"ph\": \"X\",\n"
        << "            \"args\": {\n"
        << "                \"name\": \"" << para_name << "\"\n"
...
void BytePSGlobal::OutputTraces() {
  ...
  auto tensor_ready_trace_path =
      _trace_dir + "/" + std::to_string(_local_rank) + "/tensor_ready.json";
  std::ofstream tensor_ready_file;
  tensor_ready_file.open(tensor_ready_trace_path);
  tensor_ready_file << "{" << std::endl;
  tensor_ready_file << "    \"TensorReadyTraceEvents\": [" << std::endl;
  ...
  for (std::unordered_map<std::string, int>::iterator iter = _name2end.begin();
    ...
    while (ctxt->comm_time.size() > 0) {
      ...
      BytePSGlobal::EmitTensorReadyTrace(&tensor_ready_file, ret, ctxt);
      ctxt->comm_time.pop();
    }
  ...
  tensor_ready_file << "\n" << std::endl;
  tensor_ready_file << "    ]," << std::endl;
  tensor_ready_file << "    \"displayTimeUnit\": \"ms\"" << std::endl;
  tensor_ready_file << "}" << std::endl;
