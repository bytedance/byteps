...
void NcclGroupEntry::RecordEvents() {
    for (size_t i = 0; i < tasks.size(); i++) {
        cudaEvent_t event;
        CUDA_CALL(cudaEventCreateWithFlags(&event, cudaEventBlockingSync | cudaEventDisableTiming));
        CUDA_CALL(cudaEventRecord(event,
            BytePSGlobal::GetNccl()->GetStream(tasks[i]->key, queues[i]->getQueueType())));
...
//
// broadcast
// 1st ring, cpubuff->7->6->5->4->cpubuff->3->2->1->0, cpubuff->3->2->1->0->cpubuff->7->6->5->4
// 2nd ring, cpubuff->4->7->6->5->cpubuff->0->3->2->1, cpubuff->0->3->2->1->cpubuff->4->7->6->5
