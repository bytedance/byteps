...
enum QueueType {
  ...
  BROADCAST,
  // zhuang: QueueType for intra communication
  COORDINATE_INTRA_REDUCE,
  INTRA_REDUCE,
  COORDINATE_INTRA_GATHER,
  INTRA_GATHER,
  COORDINATE_INTRA_BROADCAST,
  INTRA_BROADCAST,
  COORDINATE_INTRA_REDUCESCATTER,
  INTRA_REDUCESCATTER,
  COORDINATE_INTRA_ALLGATHER,
  INTRA_ALLGATHER,
  COORDINATE_INTRA_ALLTOALL,
  INTRA_ALLTOALL,
...
const std::vector<std::string> LogStrings = {"COORDINATE_REDUCE",
                                             ...
                                             "COORDINATE_BROADCAST",
                                             "BROADCAST",
                                             // zhuang: log strings for intra communication
                                             "COORDINATE_INTRA_REDUCE",
                                             "INTRA_REDUCE",
                                             "COORDINATE_INTRA_GATHER",
                                             "INTRA_GATHER",
...
typedef struct CommTime {
  long long start_t;
  // record the time when a tensor is ready for communication
  long long tensor_ready_t;
  long long dur = 0;
  bool end = false;
  int key = -1;
  int type = -1;
...
typedef struct BytePSContext {
  ...
  // the root node for intra-node communication
  int intra_comm_root_rank = 0;
  bool CPU_compress = false;
} BPSContext;
