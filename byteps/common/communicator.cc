...
void BytePSCommSocket::startListenThread() {  // only root starts this in
  ...
  while (true) {
    ...
    switch (message.signal) {
      case NONE_READY:
        BytePSGlobal::GetNoneTable()->AddReadyCount(message.key);
        break;
      case REDUCE_READY:
        BytePSGlobal::GetReduceTable()->AddReadyCount(message.key);
        break;
      case INTRA_REDUCE_READY:
        BytePSGlobal::GetIntraReduceTable()->AddReadyCount(message.key);
        break;
      case INTRA_GATHER_READY:
        BytePSGlobal::GetIntraGatherTable()->AddReadyCount(message.key);
        break;
      case INTRA_BROADCAST_READY:
        BytePSGlobal::GetIntraBroadcastTable()->AddReadyCount(message.key);
        break;
      case INTRA_REDUCESCATTER_READY:
        BytePSGlobal::GetIntraReducescatterTable()->AddReadyCount(message.key);
        break;
      case INTRA_ALLGATHER_READY:
        BytePSGlobal::GetIntraAllgatherTable()->AddReadyCount(message.key);
        break;
      case INTRA_ALLTOALL_READY:
        BytePSGlobal::GetIntraAlltoallTable()->AddReadyCount(message.key);
        break;
