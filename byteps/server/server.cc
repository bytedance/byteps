...
void BytePSHandler(const ps::KVMeta& req_meta,
  ...
  if (type.requestType == RequestType::kCompressedPushPull) {
    if (compressor_map_.find(key) == compressor_map_.end()) {
      ...
      auto tensor_size = byteps::common::compressor::HyperParamFinder<size_t>(kwargs, "tensor_size", true,
                                            [](size_t x) { return x >= 0; });
      // size is used to allocate the right memory size for decompression
      // size is the number of gradients 
      if (tensor_size*sizeof(float) > aligned_size) {
        aligned_size = byteps::common::Align(tensor_size*sizeof(float), common::BYTEPS_FLOAT32);
        free(stored->tensor);
        PageAlignedMalloc((void**)&stored->tensor, tensor_size*sizeof(float));
        stored->len = tensor_size*sizeof(float);
        stored->dtype = common::BYTEPS_FLOAT32;
        CHECK(stored->tensor);
      }
  ...
  if (req_meta.push) {  // push request
    ...
    if (!stored->tensor) {
      ...
      if (compressor_map_.find(key) != compressor_map_.end()) {
        type.dtype = common::BYTEPS_FLOAT32;
      }
      auto& updates = update_buf_[key];
