...
def push_pull(tensor, scope='', average=None, device_dense='', device_sparse='',
              compression=Compression.none, op=None, enable_async=False, compress=0):
    ...
    with tf.device(device_dense):
        ...
        if compress < 1:
            ...
            summed_tensors = _push_pull(tensor, scope)
            #summed_tensor = compression.decompress(summed_tensor_compressed, ctx)
        else:
            tensor_compressed, ctx = compression.send_step(tensor)
            if tensor_compressed:
                name = 'BytePSPushPull_%s' % _normalize_name(tensor.name)
                summed_tensors_compressed = _push_pull(tensor_compressed, scope, name=name)
                summed_tensors = compression.receive_step(summed_tensors_compressed, ctx)
            else:
                return None
        ...
        if not enable_async:
            ...
            if compress < 1:
                return (_div(summed_tensors, byteps_size)
                          if op == Average else summed_tensors)
            else:
                return [_div(summed_tensor, byteps_size)
                          if op == Average else summed_tensor
                          for summed_tensor in summed_tensors]
        else: # no need to average for async training
            new_tensor = summed_tensors
    return new_tensor
...
if _LegacyOptimizer is not None:
    class _DistributedOptimizer(_LegacyOptimizer):
        ...
        def __init__(self, optimizer, name=None, use_locking=False, device_dense='',
            ...
            self.iterations = 0
            self.compression_start_iteration = 1
            #self.uncompression_frequency = max(compress_config[1], 0)
            ...
            def _compression_iteration():
                return self.iterations > self.compression_start_iteration
                    ...
                    for grad in grads:
                        if grad is not None:
                            if self.iterations == 1:
                                self._compression.memory.add_tensor(grad)
                            
                            ps_grad = push_pull(grad, scope,
                                    device_dense=self._device_dense,
                                    device_sparse=self._device_sparse,
                                    compression=self._compression,
                                    enable_async=self._enable_async,
                                    compress = _compression_iteration()
                            )
                            if ps_grad is not None:
                                if isinstance(ps_grad, list):
                                    ps_grads.extend(ps_grad)
                                else:
                                    ps_grads.append(ps_grad)
                        else:
                            ps_grads.append(None)
                    return ps_grads
        ...
        def apply_gradients(self, *args, **kwargs):
            self.iterations += 1
            print("[apply gradient]", self.iterations)
...
if hasattr(tf, 'GradientTape'):
    class _DistributedGradientTape(tf.GradientTape):
        def __init__(self, tape, device_dense, device_sparse, compression, sparse_as_dense, op,
            ...
            def _compression_iteration():
                return iterations > self.compression_start_iteration
                ...
                with tf.name_scope(self._name + "_Push_Pull") as scope:
                    ...
                    for grad in grads:
                        if grad is not None:
                            ...
                            ps_grads.append(None)
                    print("[push pull grad]", len(ps_grads))
                    return ps_grads
