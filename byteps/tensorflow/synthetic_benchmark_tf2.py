...
from mergeComp.helper import add_parser_arguments, set_compressor
...
#opt, comp = wrap_compress_optimizer(model, opt, args)

data = tf.random.uniform([args.batch_size, 224, 224, 3])
target = tf.random.uniform([args.batch_size, 1], minval=0, maxval=999, dtype=tf.float64)

# BytePS: (optional) compression algorithm.
compressor = set_compressor(model, args)
from sparse_optimizer import DistributedGradientTape as dgt
...
def benchmark_step(first_batch):
    ...
    tape = dgt(tape, compression=compressor)
