...
namespace {
    ...
    public:
        Array2D(size_t rows, size_t row_size)
            : _rows(rows), 
              ...
              _data{new float[rows * row_size]} {}

        size_t rows() const { return _rows; }
        size_t row_size() const {return _row_size; }
        size_t total_size() const {return _total_size; }
        float *data() { return _data.get(); }
        void reset() { std::memset(_data.get(), 0, _rows * _row_size * sizeof(float)); }
    ...
    public:
        ...
        void write_row(size_t row, float *array, size_t row_size) {
        ...
        void read_row(size_t row, float *array, size_t row_size) {
            auto pointer = _tensor[row];
            for (auto i=0; i<row_size; i++) {
                array[i] = pointer[i];
            }
...
std::vector<type_t> CPU_overhead(size_t sparse_rows, size_t dense_rows) {
    int row_size = 256;

    int runs = 100, skip=10;
    ...
    for (int servers: servers_list) {
        ...
        for (float density: densities) {
            ...
            std::cout << "dense rows: " << dense_tensor_rows
                ...
                << std::endl;
            auto ret = ::byteps::common::CPU_overhead<float>(sparse_tensor_rows, dense_tensor_rows_each_server);
