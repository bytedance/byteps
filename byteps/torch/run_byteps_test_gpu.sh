...
# BYTEPS_INTRA_COMPRESSOR should be set to "" for two-layer compression
#export BYTEPS_INTRA_COMPRESSOR=""
...
#export PS_VERBOSE=${PS_VERBOSE:-2}
...
if [ $1 == "worker" ] || [ $1 == "joint" ]; then
  export DMLC_ROLE=$1
  echo "TEST TORCH ..."
  bpslaunch python3.7 $path/benchmark_byteps.py --model vgg16 --num-iters 5 --compress --compressor ${compressor} --memory none --comm byteps_gpu_compress --compress-ratio ${compress_ratio} --fusion-num $3
