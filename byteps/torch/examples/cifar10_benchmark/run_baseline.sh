...
export DMLC_PS_ROOT_URI="10.130.19.201"
...
#     pkill -9 python3
#     BENCHMARK_ARGS="--compress --compressor none --memory none --comm bytecomp --compress-ratio ${compress_ratio} --scheduler-file none --scheduler-type 0"
#     python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/cifar10_benchmark.py --model ${model} --epochs 2 --batch-size 32 ${BENCHMARK_ARGS} --speed_test 
#   done
# done
...
do
  ...
  do 
    ...
    #     $GDB python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/cifar10_benchmark.py --model ${model} --epochs 3 --batch-size 32 --speed_test $BENCHMARK_ARGS | tee -a logs
    #     sleep 2
    #   done
