...
do
  ...
  BENCHMARK_ARGS="--compress --compressor ${compressor} --memory efsignsgd --comm byteps-compress --compress-ratio ${compress_ratio} --model-name ${model}"
  python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/main.py --model ${model} --epochs 2 $BENCHMARK_ARGS
  sleep 5
done
...
do
  pkill -9 python3
  export BYTEPS_INTER_COMPRESSOR=${compressor}
  BENCHMARK_ARGS="--compress --compressor ${compressor} --memory efsignsgd --comm hitopkcomm --compress-ratio ${compress_ratio} --model-name ${model}"
  python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/main.py --model ${model} --epochs 2 $BENCHMARK_ARGS
  sleep 5
done