...
dirname="baseline_results"
mkdir $dirname
# FP32/FP16 baseline
for pcie in 0 1 
do
    export NCCL_P2P_DISABLE=${pcie}
    BENCHMARK_ARGS="--compress --compressor none --memory none --comm bytecomp --compress-ratio ${compress_ratio} --scheduler-file 0 --scheduler-type 0"
    python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/main.py --model ${model} --epochs 1 | tee -a ${dirname}/pcie_${pcie}_${DMLC_NUM_WORKER}
    BENCHMARK_ARGS="--compress --compressor none --memory none --comm bytecomp --compress-ratio ${compress_ratio} --scheduler-file 0 --scheduler-type 1"
    python3 -m torch.distributed.launch $DISTRIBUTED_ARGS $path/main.py --model ${model} --epochs 1 --fp16 | tee -a ${dirname}/pcie_${pcie}_fp16_${DMLC_NUM_WORKER}
done
...
hdfs dfs -copyFromLocal ${dirname} hdfs://haruna/byte_search/user/zhuangwang/bytecomp/lstm/${dirname}