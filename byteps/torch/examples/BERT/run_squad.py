...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        if example_index > examples_limit:
            break
...
def main():
    ...
    parser.add_argument("--examples_limit",
                        default=100000,
                        type=int,
                        help="limit the number of examples loaded in convert_examples_to_features()")
    parser = add_parser_arguments(parser)
    args = parser.parse_args()
    init_comm(args.local_rank)
    args.world_size = bps.size()
    args.rank = bps.rank()
    ...
    print("device: {} n_gpu: {}, distributed training: {}, 16-bits training: {}".format(
                                device, n_gpu, bool(args.local_rank != -1), args.amp_fp16))
    ...
    if args.do_train:
        ...
        if args.local_rank != -1:
            num_train_optimization_steps = num_train_optimization_steps // bps.size()
    ...
    if args.do_train: 
        ...
        if args.local_rank == -1:
            ...
            train_sampler = DistributedSampler(train_data, num_replicas=bps.size(), rank=bps.rank())
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                total_time += time.time() - step_time
                final_loss = loss.item()
                if bps.local_rank() == 0 and step % args.log_freq == 0:
                    # in ms
    ...
    if args.do_predict and (args.local_rank == -1 or is_main_process()):
        ...
        if not args.do_train and args.amp_fp16:
            model.half()
