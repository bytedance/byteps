...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a NLP model")
    # Required parameters
    parser.add_argument(
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    train_sampler = DistributedSampler(train_dataset, num_replicas=bps.size(), rank=bps.rank())
    ...
    for _ in train_iterator:
        ...
        start_idx = int(len(epoch_iterator) * 0.25)
        end_idx = int(len(epoch_iterator) * 0.75)
