...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters, compression, compress_config=(1, 0),
        ...
        self._iterations = 0
        self._compression_start_iteration = max(compress_config[0], 1)
        self._tensor_info = []
        self._collect_tensor_info = (int(os.getenv('BYTEPS_TENSOR_INFO', 0)) != 0)
    ...
    def _communicate_grad_async(self, p):
        ...
        if self._collect_tensor_info and self._iterations == 0:
            self._tensor_info.append((name, tensor.numel()))

        if self._collect_tensor_info and self._iterations > 0:
            if rank() == 0:
                print(self._tensor_info, flush=True)
            self._collect_tensor_info = False

        # TODO: handle _enable_async
        handle, ctx = None, None
        if self._compression_iteration():
    ...
    def synchronize(self):
        ...
        for p, (handles, ctx) in self._handles.items():
            self._push_pull_delay[p] = self.backward_passes_per_step
            if self._compression_iteration() and ctx is not None:
...
def DistributedOptimizer(optimizer, named_parameters=None,
    ...
    return cls(optimizer.param_groups, named_parameters,
               compression, compress_config, backward_passes_per_step)
