...
class SyncBatchnormFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            ...
            if world_size > 1:
                mean_dy_name = 'syncbn_mean_dy.' + str(torch.numel(mean_dy)) + '.' + str(mean_dy.dtype)
                mean_dy_xmu_name = 'syncbn_mean_dy_xmu.' + str(torch.numel(mean_dy_xmu)) + '.' + str(mean_dy_xmu.dtype)
                mean_dy_handle = push_pull_async(
                    mean_dy, average=False, name=mean_dy_name)
                mean_dy_xmu_handle = push_pull_async(
                    mean_dy_xmu, average=False, name=mean_dy_xmu_name)
